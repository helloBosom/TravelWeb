       ...
       <c:choose>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
           ...
           <c:forEach var="v"  items="${orders}">
			<input type="button" value="通过" name="B332" class="long" onClick="window.location='<%=path%>/order/${v[0]}/checkorderbyid.do';"/> 
			<input type="button" value="不通过" name="B333" class="long" onClick="window.location='<%=path%>/order/${v[0]}/nocheckorderbyid.do';"/>
			<input type="button" value="返回" name="B331" class="long" onClick="window.location='<%=path%>/order/queryallorder.do';" />
		 	</c:forEach>
		 </td>
